...
TraitEngine = import_module("petri_dish.traits").TraitEngine
TraitVector = import_module("petri_dish.traits").TraitVector
...
def test_observe_death_rebirth() -> None:
    engine = TraitEngine(Settings(traits_rebirth_factor=0.85))
    traits = engine.get_traits("alice")
...
def test_end_round_decay() -> None:
    engine = TraitEngine(Settings(traits_decay_factor=0.95))
    traits = engine.get_traits("alice")
...
def test_generate_instincts_empty() -> None:
    engine = TraitEngine(Settings(traits_instinct_threshold=0.3))
